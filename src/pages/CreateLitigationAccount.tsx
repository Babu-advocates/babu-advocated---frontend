...
const CreateLitigationAccount = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("litigation_accounts").insert([
        {
          username: formData.username,
          password: formData.password,
          litigation_name: "", // Empty string as field is not needed
      ...
      toast.success("Litigation account created successfully");
      navigate("/admin/litigation-accounts");
    } catch (error: any) {
      toast.error(error.message || "Failed to create litigation account");
    } finally {
      setLoading(false);
    }
  };
...
export default CreateLitigationAccount;
